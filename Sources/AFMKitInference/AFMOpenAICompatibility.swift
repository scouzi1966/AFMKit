...
public extension AFMRequest {
    init(openAIMessages: [Message], generationConfig: GenerationConfig) throws {
        ...
        let tools = try (generationConfig.tools ?? []).map { tool in
            guard tool.type == "function" else {
                throw AFMError.invalidRequest("Unsupported tool type '\(tool.type)'.")
            }
            return AFMToolDefinition(
        ...
        let responseConstraint = try generationConfig.responseFormat?.afmConstraint()
        self.init(
            messages: messages,
            tools: tools,
            options: AFMGenerationOptions(
                ...
                reasoningEnabled: generationConfig.reasoningEnabled,
    ...
    init(openAIMessage message: Message) throws {
        ...
        self.init(
            ...
            toolCalls: (message.toolCalls ?? []).map {
                AFMToolCall(id: $0.id, name: $0.function.name, arguments: $0.function.arguments)
            }
        )
    }
}
    ...
    init(openAIContentPart part: ContentPart) throws {
        ...
        case "image_url":
            ...
            if rawValue.hasPrefix("data:") {
                guard let parsed = Self.parseDataURL(rawValue) else {
                    throw AFMError.invalidRequest("Invalid image data URL.")
                }
                self = .data(mimeType: parsed.mimeType, value: parsed.data)
            } else if let url = URL(string: rawValue), url.scheme != nil {
                self = .reference(url)
            } else {
...
private extension AnyCodableValue {
    var afmJSONValue: AFMJSONValue {
        ...
        case .array(let values): return .array(values.map(\.afmJSONValue))
        case .object(let values): return .object(values.mapValues(\.afmJSONValue))
        }
    }
}

private extension ResponseFormat {
    func afmConstraint() throws -> AFMResponseConstraint? {
        switch type {
        case "text": return nil
        case "json_object": return .jsonObject
        case "json_schema":
            guard let schema = jsonSchema?.schema?.value.afmJSONValue else {
                throw AFMError.invalidRequest("json_schema response format requires a schema.")
            }
            return .jsonSchema(
        ...
        default:
            throw AFMError.invalidRequest("Unsupported response format '\(type)'.")
        }
    }
}

public extension AFMResponse {
