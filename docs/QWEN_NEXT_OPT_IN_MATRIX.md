...
Inventory introduced **2026-09-14** at AFMKit `49a97c7a` / consumer `9acccfc`;
boundary controls added **2026-09-18** on top of AFMKit `37532c2b` / consumer
`e8b8e7a`. Workstream: [PR #123](https://github.com/scouzi1966/AFMKit/pull/123).
...
| N8 retained lifecycle | Frozen default-normalization binary, M25/C15/prefix; long mixed-lane prompts and request identities | 565/565 assertions (corrected from the original prose's 665), 182 shared-verification batches, cancellation recovery/full replay, clean exit | Measured; actual model A→B→A remains untested; [report](QWEN_NEXT_RETAINED_QUALIFICATION.md) |
| N9 retained C15 presets | A9 AR versus M25 MTP and preserved reference; identical broader-gate payloads, 15-prompt replay windows | 360/360 runtime/structure/identity. Sampled repeats AFM AR 154.36 at 23/30, MTP 179.69 at 21/30; reference AR 129.20 at 23/30, MTP 91.85 at 22/30 | No defaults promoted; AFM first-use paths remain slower; [report](QWEN_NEXT_RETAINED_QUALIFICATION.md) |
| N10 single-client scheduler replay | Full M25/prefix preset; change server `--concurrent 1` to `2`, client concurrency stays one | 180/180 runtime/structure/identity; 90/90 paired texts exact. Sampled repeats 47.95→120.70 wall tok/s, 22/30 each; first-use 48.05→47.09. Full prompt hits only with scheduler | Existing explicit opt-in, no new serial implementation or automatic routing; [tradeoffs](QWEN_NEXT_RETAINED_QUALIFICATION.md#one-client-through-the-existing-scheduler) |
...
| A11 | A9 + `AFM_QWEN_PREFIX_REPLAY_BACKOFF=31`, C15/prefix/window 15 | Sampled first 87.60→119.92 aggregate tok/s; repeat 159.24→158.67. Correct 23→24 of 30, but one new wrong answer shape. Experimental, not quality-qualified. [Evidence](QWEN_NEXT_PREFIX_BOUNDARY_EXPERIMENT.md). |
...
| M26 | M25 + `AFM_QWEN_MTP_REPLAY_BACKOFF=31`, C15/prefix/window 15 | Sampled first 53.61→117.70/136.70 tok/s, but exact repeats 182.60→143.34/147.42. Initial cold delay preserved; prefix-only policy not recommended for repeat-heavy work. [Evidence](QWEN_NEXT_PREFIX_BOUNDARY_EXPERIMENT.md). |
| M27 | M26 + `AFM_QWEN_MTP_REPLAY_BACKOFF_ON_MISS=1` | Initial refinement: 135.57/178.36 tok/s; lifecycle exposed unshared-prefix/endpoint LRU duplication. Fixed ownership: first/repeat 137.94/182.75 tok/s, correct 23/22 of 30; 565/565 lifecycle assertions. Greedy repeat remains below endpoint control; no default promotion. [Evidence](QWEN_NEXT_PREFIX_BOUNDARY_EXPERIMENT.md). |
| M28 | M27 + `AFM_QWEN_MTP_REPLAY_ANCHOR=1` | Default-off retention experiment: protect one earlier boundary with distinct-source coverage, within existing entry/byte caps. Two reversed-order A/B pairs: greedy second-repeat 162.50→185.10 and 160.15→186.90 aggregate tok/s; complete endpoint hits 8/7→15. Sampled timing mixed; totals unchanged but pair B swaps one semantic pass/fail in every phase. 95 targeted Release tests pass, one optional skip; 565 lifecycle assertions pass. **Not quality-neutral or a default recommendation.** [Refresh and diagnosis](QWEN_NEXT_REFERENCE_REFRESH.md). |
| M29 | M25/W8-L2 shared verifier + `AFM_QWEN_MTP_SHARED_DEFER_PLE_IDS=1`, C15, prefix explicitly disabled | Defers the eager draft-ID CPU read to mapped PLE's existing private-leaf flush. Two reversed-order 45-request pairs: greedy aggregate +2.25% and +0.48%; sampled +4.10% in pair one, while pair two had one stochastic control-length change and no clean aggregate-rate claim. 180/180 runtime and structure; 89/90 paired texts identical, with the differing sampled text occurring between the two controls as well. Peak RSS changed by less than 17 MiB. **Retain opt-in; gain is not stable enough for default promotion.** |
...
| `AFM_QWEN_PREFIX_REPLAY_BACKOFF` | 0 / existing coarse checkpoint grid | Qwen Next scheduler AR lane only, prefix enabled. Integer clamped 0–256; `31` replaces the interior grid with one near-end boundary while retaining the final prompt-minus-one snapshot. Still respects prefill chunk bounds. Experimental, other models/serial paths unchanged. |
...
| `AFM_QWEN_MTP_REPLAY_BACKOFF` | 0 / endpoint snapshot | Experimental earlier complete-state boundary, integer clamped 0–256; invalid/unset 0. Requires Qwen MTP owner + prefix + positive replay budget. `31` matches the reference's 30 held-back prefill tokens plus final token. Replaces the endpoint snapshot, not an extra cache: repeats recompute that tail. Short prompts keep their endpoint. Qualification in progress; no default recommendation. |
| `AFM_QWEN_MTP_REPLAY_BACKOFF_ON_MISS` | Off | `1` + positive backoff: misses seed an earlier snapshot; hits save the endpoint. Promotion replaces an unshared original-prompt boundary; actually shared boundaries stay separately budgeted. Bounded/accounted source ownership metadata, no new cache or budget. Qualification in progress. |
| `AFM_QWEN_MTP_REPLAY_ANCHOR` | Off | `1` with M27: best-effort protection of one earlier donor selected by distinct-source coverage, then depth. Insertion-only policy, no new snapshot/capacity. No eligible donor keeps prior behavior; byte limits can evict the anchor. Experimental, not a general recommendation. |
| `AFM_QWEN_MTP_SHARED_VERIFY` | Off | `1` + owner; actual sharing requires eligible batched-policy sessions |
| `AFM_QWEN_MTP_SHARED_DEFER_PLE_IDS` | Off | `1` + shared verifier; omits the early combined draft-ID host read and lets mapped PLE resolve IDs at its existing private-leaf flush. Experimental M29; ordinary AR, independent MTP and defaults are unchanged. |
