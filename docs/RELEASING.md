...
| --- | --- | --- |
| `AFMKit` | `AFMKitCore`, `AFMOpenAICompat`, `AFMKitInference`, `AFMKitApple` | None; root manifest has zero package dependencies. |
...
3. `Scripts/validate-release.sh` validates all three manifests, both provider
   locks, all seven API baselines, Release tests, workflow/security regressions,
...
4. The downstream gate materializes provider release repositories in private
   ...
   provider locks, and builds all seven products from the exact release manifests.
