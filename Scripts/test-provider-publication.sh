...
publish_fixture > "$SANDBOX/first.log"
for package_name in AFMKitDwarfStar AFMKitMLX; do
    case "$package_name" in
...
publish_fixture > "$SANDBOX/recovery.log"
grep -q "already published at the qualified revision" "$SANDBOX/recovery.log"
...
echo "3 provider publication regression scenarios passed."
