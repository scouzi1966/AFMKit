...
HOSTED_FOUNDATION_MODELS_UNSTABLE_CASES=(
    ...
    "AFMKitAppleTests.FoundationStopSequenceFilterTests/testAcceptedTranscriptExcludesHiddenPostStopOutput"
    "AFMKitAppleTests.FoundationModelSessionCoordinatorTests/testDynamicProfileSessionReusesExactProviderAndSignature"
    "AFMKitAppleTests.FoundationModelSessionCoordinatorTests/testSimpleSessionReusesExactProviderAndSignature"
    ...
    "AFMKitAppleTests.FoundationSessionUsageTelemetryTests/testMapsLanguageModelSessionUsageIntoTelemetry"
    "AFMKitAppleTests.FoundationSessionUsageTelemetryTests/testSingleResponseTelemetryUsesCompletionAsFirstChunk"
    ...
    "AFMKitAppleTests.FoundationTranscriptSnapshotParserTests/testOutputWithoutMatchingIDUsesLatestRequestedToolWithSameName"
    "AFMKitAppleTests.FoundationTranscriptSnapshotParserTests/testReasoningContentReturnsReasoningAfterLatestPrompt"
    "AFMKitAppleTests.FoundationStructuredResponseCompleterTests/testCompletesRenderedContentToolSnapshotsAndTelemetry"
)
...
is_case_isolated_target() {
    local test_target="$1"
    local isolated_target

    for isolated_target in "${CASE_ISOLATED_TARGETS[@]}"; do
        [[ "$test_target" == "$isolated_target" ]] && return 0
    done
    return 1
}
...
run_case_isolated_test() {
    ...
    while true; do
        ...
        attempt=$((attempt + 1))
        echo "Retrying Xcode 27 beta Apple framework case after signal 11 ($attempt/$FOUNDATION_MODELS_SIGNAL_RETRY_LIMIT): $test_case" >&2
    done
}
...
while IFS= read -r test_target; do
    ...
    if is_case_isolated_target "$test_target"; then
