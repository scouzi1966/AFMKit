...
public enum TTS {
    ...
    public static func loadModel(
        ...
        switch resolvedType {
        case "qwen3_tts":
            return try await Qwen3TTSModel.fromPretrained(modelRepo, cache: cache, downloadIfNeeded: downloadIfNeeded)
        case "qwen3", "qwen":
            return try await Qwen3Model.fromPretrained(modelRepo, cache: cache, downloadIfNeeded: downloadIfNeeded)
        case "llama_tts", "llama3_tts", "llama3", "llama", "orpheus", "orpheus_tts":
            return try await LlamaTTSModel.fromPretrained(modelRepo, cache: cache, downloadIfNeeded: downloadIfNeeded)
        case "csm", "sesame":
            return try await MarvisTTSModel.fromPretrained(modelRepo, cache: cache, downloadIfNeeded: downloadIfNeeded)
        case "soprano_tts", "soprano":
            return try await SopranoModel.fromPretrained(modelRepo, cache: cache, downloadIfNeeded: downloadIfNeeded)
        case "pocket_tts":
            return try await PocketTTSModel.fromPretrained(modelRepo, cache: cache, downloadIfNeeded: downloadIfNeeded)
        default:
