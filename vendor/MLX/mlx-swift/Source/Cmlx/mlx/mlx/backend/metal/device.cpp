...
CommandEncoder::~CommandEncoder() {
  ...
  if (profile_value != nullptr && std::string(profile_value) == "1") {
    std::fprintf(
        ...
        static_cast<unsigned long long>(committed_buffers_),
        static_cast<unsigned long long>(committed_ops_),
        static_cast<unsigned long long>(committed_bytes_));
  }
  auto pool = new_scoped_memory_pool();
  buffer_.reset();
  queue_.reset();
...
void CommandEncoder::commit(std::function<void()> completion) {
  ...
  committed_buffers_ += 1;
  committed_ops_ += buffer_ops_;
  committed_bytes_ += buffer_sizes_;
  buffer_->commit();
  buffer_ = NS::RetainPtr(queue_->commandBufferWithUnretainedReferences());
  buffer_ops_ = 0;
  buffer_sizes_ = 0;
...
CommandBufferProfile CommandEncoder::command_buffer_profile_since_report() {
  CommandBufferProfile profile{
      committed_buffers_ - reported_buffers_,
      committed_ops_ - reported_ops_,
      committed_bytes_ - reported_bytes_};
  reported_buffers_ = committed_buffers_;
  reported_ops_ = committed_ops_;
  reported_bytes_ = committed_bytes_;
  return profile;
}
