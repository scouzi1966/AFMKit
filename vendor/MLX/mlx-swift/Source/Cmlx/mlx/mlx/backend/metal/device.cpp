...
Device::~Device() {
  auto pool = new_scoped_memory_pool();
  const char* profile_value = std::getenv("MLX_PROFILE_COMMAND_BUFFERS");
  if (profile_value != nullptr && std::string(profile_value) == "1") {
    uint64_t buffers = 0;
    uint64_t ops = 0;
    uint64_t bytes = 0;
    for (const auto& [_, stream] : stream_map_) {
      buffers += stream.committed_buffers;
      ops += stream.committed_ops;
      bytes += stream.committed_bytes;
    }
    std::fprintf(
        stderr,
        "[MLXCommandBufferTotals] buffers=%llu ops=%llu bytes=%llu\n",
...
void Device::commit_command_buffer(int index) {
  auto& stream = get_stream_(index);
  stream.committed_buffers += 1;
  stream.committed_ops += stream.buffer_ops;
  stream.committed_bytes += stream.buffer_sizes;
...
CommandBufferProfile Device::command_buffer_profile_since_report(int index) {
  auto& stream = get_stream_(index);
  CommandBufferProfile profile{
      stream.committed_buffers - stream.reported_buffers,
      stream.committed_ops - stream.reported_ops,
      stream.committed_bytes - stream.reported_bytes};
  stream.reported_buffers = stream.committed_buffers;
  stream.reported_ops = stream.committed_ops;
  stream.reported_bytes = stream.committed_bytes;
  return profile;
}
