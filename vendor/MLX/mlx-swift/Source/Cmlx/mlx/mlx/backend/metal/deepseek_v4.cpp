// AFM DeepSeek V4 Metal primitives ported to MLX v0.32.2.
...
void DeepseekV4MXFP4MoE::eval_gpu(
  ...
  encoder.add_temporary(std::move(activated));
  for (auto& selected : selected_routes) {
    encoder.add_temporary(std::move(selected));
  }
  if (shared_q8_) {
    encoder.add_temporary(std::move(routed_output));
    encoder.add_temporary(std::move(shared_activated));
  }
}
...
void DeepseekV4HCQ8MoE::eval_gpu(
  ...
  encoder.add_temporary(std::move(logits));
  encoder.add_temporary(std::move(post));
  encoder.add_temporary(std::move(comb));
  encoder.add_temporary(std::move(indices));
  encoder.add_temporary(std::move(scores));
  encoder.add_temporary(std::move(activated));
  encoder.add_temporary(std::move(routed));
  encoder.add_temporary(std::move(shared_activated));
}
