...
# Keep the generated SwiftPM sources synchronized with every
# make_jit_source() declaration in the pinned MLX core.
...
# Run the command to do the build-time code generation for CUDA. Match the
# file(GLOB) used by MLX instead of carrying a second hard-coded source list.
