...
// MARK: - Fused KDA decode

/// GLM-5.3's decode geometry is fixed at 64 x 128 KDA heads with a four-tap
/// depthwise convolution. Keeping the convolution, normalization, recurrent
/// update, and gated output normalization in one dispatch avoids materializing
/// the intermediate q/k/v tensors 34 times for every generated token.
private final class GLM5NextKDADecodeKernel: Sendable {
    static let shared = GLM5NextKDADecodeKernel()
    ...
    private init() {
        let source = """
            constexpr int D = 128;
            constexpr int K = 4;
            constexpr int C = H * D * 3;
            constexpr float Q_SCALE = 0.08838834764831845f;
            ...
            threadgroup float qn[D];
            threadgroup float kn[D];
            threadgroup float vv[D];
            threadgroup float outv[D];
            threadgroup float partial[8];

            for (uint i = tid; i < 3 * D; i += 256) {
                uint channel;
                if (i < D) {
                    channel = head * D + i;
                } else if (i < 2 * D) {
                    channel = H * D + head * D + (i - D);
                } else {
                    channel = 2 * H * D + head * D + (i - 2 * D);
                }
                float conv =
                    static_cast<float>(conv_weight[channel * K + 0])
                        * static_cast<float>(conv_state[0 * C + channel])
                    + static_cast<float>(conv_weight[channel * K + 1])
                        * static_cast<float>(conv_state[1 * C + channel])
                    + static_cast<float>(conv_weight[channel * K + 2])
                        * static_cast<float>(conv_state[2 * C + channel])
                    + static_cast<float>(conv_weight[channel * K + 3])
                        * static_cast<float>(projected[channel]);
                float activated = conv / (1.0f + metal::exp(-conv));
                float rounded = static_cast<float>(static_cast<InT>(activated));

                conv_state_out[0 * C + channel] = conv_state[1 * C + channel];
                conv_state_out[1 * C + channel] = conv_state[2 * C + channel];
                conv_state_out[2 * C + channel] = projected[channel];
            ...
            float square = 0.0f;
            if (tid < D) {
                square = qn[tid] * qn[tid];
            } else if (tid < 2 * D) {
                square = kn[tid - D] * kn[tid - D];
            }
            square = simd_sum(square);
            if (lane == 0) partial[sg] = square;
            threadgroup_barrier(mem_flags::mem_threadgroup);
            float q_sum = partial[0] + partial[1] + partial[2] + partial[3];
            float k_sum = partial[4] + partial[5] + partial[6] + partial[7];
            float q_inv = metal::rsqrt(q_sum + 1e-6f) * Q_SCALE;
            float k_inv = metal::rsqrt(k_sum + 1e-6f);
            if (tid < D) {
                qn[tid] *= q_inv;
            } else if (tid < 2 * D) {
                kn[tid - D] *= k_inv;
            ...
            float beta_value = 1.0f /
                (1.0f + metal::exp(-static_cast<float>(beta_raw[head])));
            float a_log_value = metal::exp(static_cast<float>(a_log[head]));
            float bound = static_cast<float>(lower_bound);

            for (uint dv = sg; dv < D; dv += 8) {
                const uint state_row = (head * D + dv) * D;
                const uint gate_base = head * D + 4 * lane;
                float s0 = static_cast<float>(delta_state[state_row + 4 * lane + 0]);
                float s1 = static_cast<float>(delta_state[state_row + 4 * lane + 1]);
                float s2 = static_cast<float>(delta_state[state_row + 4 * lane + 2]);
                float s3 = static_cast<float>(delta_state[state_row + 4 * lane + 3]);
                float u0 = a_log_value *
                    (static_cast<float>(gate_raw[gate_base + 0])
                        + static_cast<float>(dt_bias[gate_base + 0]));
                float u1 = a_log_value *
                    (static_cast<float>(gate_raw[gate_base + 1])
                        + static_cast<float>(dt_bias[gate_base + 1]));
                float u2 = a_log_value *
                    (static_cast<float>(gate_raw[gate_base + 2])
                        + static_cast<float>(dt_bias[gate_base + 2]));
                float u3 = a_log_value *
                    (static_cast<float>(gate_raw[gate_base + 3])
                        + static_cast<float>(dt_bias[gate_base + 3]));
                s0 *= metal::exp(bound / (1.0f + metal::exp(-u0)));
                s1 *= metal::exp(bound / (1.0f + metal::exp(-u1)));
                s2 *= metal::exp(bound / (1.0f + metal::exp(-u2)));
                s3 *= metal::exp(bound / (1.0f + metal::exp(-u3)));

                float k0 = kn[4 * lane + 0];
                float k1 = kn[4 * lane + 1];
                float k2 = kn[4 * lane + 2];
                float k3 = kn[4 * lane + 3];
                float memory = s0 * k0 + s1 * k1 + s2 * k2 + s3 * k3;
                memory = simd_sum(memory);
                float delta = (vv[dv] - memory) * beta_value;
                s0 += k0 * delta;
                s1 += k1 * delta;
                s2 += k2 * delta;
                s3 += k3 * delta;
                ...
                float result =
                    s0 * qn[4 * lane + 0] + s1 * qn[4 * lane + 1]
                    + s2 * qn[4 * lane + 2] + s3 * qn[4 * lane + 3];
                result = simd_sum(result);
                delta_state_out[state_row + 4 * lane + 0] = static_cast<StT>(s0);
                delta_state_out[state_row + 4 * lane + 1] = static_cast<StT>(s1);
                delta_state_out[state_row + 4 * lane + 2] = static_cast<StT>(s2);
                delta_state_out[state_row + 4 * lane + 3] = static_cast<StT>(s3);
                if (lane == 0) {
                    // The staged path rounds the recurrent output back to the
                    // activation dtype before gated RMS normalization.
                    outv[dv] = static_cast<float>(static_cast<InT>(result));
                }
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);

            float energy = tid < D ? outv[tid] * outv[tid] : 0.0f;
            energy = simd_sum(energy);
            if (lane == 0 && sg < 4) partial[sg] = energy;
            threadgroup_barrier(mem_flags::mem_threadgroup);
            float inverse_rms = metal::rsqrt(
                (partial[0] + partial[1] + partial[2] + partial[3]) / float(D)
                    + static_cast<float>(norm_epsilon));
            if (tid < D) {
                const uint index = head * D + tid;
                float gate_value = 1.0f /
                    (1.0f + metal::exp(-static_cast<float>(output_gate[index])));
                output[index] = static_cast<InT>(
                    outv[tid] * inverse_rms
                        * static_cast<float>(norm_weight[tid]) * gate_value);
            }
        """
        kernel = MLXFast.metalKernel(
            name: "glm5_next_kda_decode",
            inputNames: [
                "projected", "output_gate", "gate_raw", "beta_raw",
                "conv_state", "conv_weight", "a_log", "dt_bias",
                "delta_state", "norm_weight", "lower_bound", "norm_epsilon",
            ],
            outputNames: ["output", "conv_state_out", "delta_state_out"],
            source: source)
    }
}

func glm5NextFusedKDADecode(
    projected: MLXArray,
    outputGate: MLXArray,
    gateRaw: MLXArray,
    betaRaw: MLXArray,
    ...
    lowerBound: Float,
    normEps: Float
) -> (output: MLXArray, convState: MLXArray, deltaState: MLXArray)? {
    let heads = 64
    let dimension = 128
    let channels = heads * dimension * 3
    let activationDType = projected.dtype
    let activationDTypeIsSupported = activationDType == .bfloat16
    guard Device.defaultDevice().deviceType == .gpu,
          activationDTypeIsSupported,
          projected.shape == [1, 1, channels],
          outputGate.shape == [1, 1, heads, dimension],
          gateRaw.shape == [1, 1, heads, dimension],
          betaRaw.shape == [1, 1, heads],
          convState.shape == [1, 3, channels],
          convWeight.shape == [channels, 4, 1],
          aLog.size == heads,
          dtBias.size == heads * dimension,
          deltaState.shape == [1, heads, dimension, dimension],
          normWeight.size == dimension,
          outputGate.dtype == activationDType,
          gateRaw.dtype == activationDType,
          betaRaw.dtype == activationDType,
          convState.dtype == activationDType,
          convWeight.dtype == activationDType,
          normWeight.dtype == activationDType,
          aLog.dtype == .float32,
          dtBias.dtype == .float32,
          deltaState.dtype == .float32,
          let kernel = GLM5NextKDADecodeKernel.shared.kernel else {
        return nil
    }

    let outputs = kernel(
        [
            projected, outputGate, gateRaw, betaRaw, convState, convWeight,
            aLog, dtBias, deltaState, normWeight, MLXArray(lowerBound),
            MLXArray(normEps),
        ],
        template: [
            ("InT", projected.dtype),
            ("StT", deltaState.dtype),
            ("H", heads),
        ],
        grid: (256, 1, heads),
        threadGroup: (256, 1, 1),
        ...
        outputDTypes: [activationDType, convState.dtype, deltaState.dtype])
...
private final class GLM5NextLinearAttention: Module {
    ...
    func callAsFunction(
        ...
        if length == 1,
           ...
           let lowerBound = forgetGate.lowerBound
        {
            ...
            if let fused = glm5NextFusedKDADecode(
                projected: projected,
                outputGate: gate,
                gateRaw: a,
                betaRaw: b,
                ...
                aLog: forgetGate.aLog,
                dtBias: forgetGate.dtBias,
                deltaState: recurrentState,
                normWeight: outputNorm.weight,
                lowerBound: lowerBound,
                normEps: outputNorm.eps)
            {
                cache[0] = fused.convState
                cache[1] = fused.deltaState
                cache.offset += length
                return outputProj(fused.output.reshaped(batch, length, projectedSize))
            }
        }
