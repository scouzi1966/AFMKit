...
final class AFMMLXVisionAssetQualificationTests: XCTestCase {
    ...
    func testStandaloneSafetensorShardsRejectDuplicateNormalizedVisionTensor() throws {
        let directory = try makeModelDirectory(indexed: false)
        try Self.writeSafetensorHeader(
            tensorNames: Self.requiredVisionTensorNames(depth: 2),
            metadata: Self.requiredVisionTensorMetadata(depth: 2),
            to: directory.appendingPathComponent("weights-1.safetensors")
        )
        try Self.writeSafetensorHeader(
            tensorNames: ["model.visual.patch_embed.proj.weight"],
            metadata: ["model.visual.patch_embed.proj.weight": ("F16", [32, 2, 2, 2, 3])],
            to: directory.appendingPathComponent("weights-2.safetensors")
        )
    ...
    func testIndexedSafetensorRejectsDuplicateNormalizedVisionTensorInSameShard() throws {
        let directory = try makeModelDirectory(indexed: false)
        let required = Self.requiredVisionTensorNames(depth: 2)
        let alias = "model.visual.patch_embed.proj.weight"
        let allNames = required.union([alias])
        let shard = "model-00001-of-00001.safetensors"
        try Self.writeJSON(
            ["weight_map": Dictionary(uniqueKeysWithValues: allNames.map { ($0, shard) })],
            to: directory.appendingPathComponent("model.safetensors.index.json")
        )
        var metadata = Self.requiredVisionTensorMetadata(depth: 2)
        metadata[alias] = ("F16", [32, 2, 2, 2, 3])
        try Self.writeSafetensorHeader(
            tensorNames: allNames,
            metadata: metadata,
            to: directory.appendingPathComponent(shard)
        )

        XCTAssertFalse(try qualify(directory).isAssetUsable)
    }

    func testIndexedSafetensorRejectsDuplicateNormalizedVisionTensorAcrossShards() throws {
        let directory = try makeModelDirectory(indexed: false)
        let required = Self.requiredVisionTensorNames(depth: 2)
        let alias = "model.visual.patch_embed.proj.weight"
        let firstShard = "model-00001-of-00002.safetensors"
        let secondShard = "model-00002-of-00002.safetensors"
        var weightMap = Dictionary(uniqueKeysWithValues: required.map { ($0, firstShard) })
        weightMap[alias] = secondShard
        try Self.writeJSON(
            ["weight_map": weightMap],
            to: directory.appendingPathComponent("model.safetensors.index.json")
        )
        try Self.writeSafetensorHeader(
            tensorNames: required,
            metadata: Self.requiredVisionTensorMetadata(depth: 2),
            to: directory.appendingPathComponent(firstShard)
        )
        try Self.writeSafetensorHeader(
            tensorNames: [alias],
            metadata: [alias: ("F16", [32, 2, 2, 2, 3])],
            to: directory.appendingPathComponent(secondShard)
        )
    ...
    func testSnapshotFingerprintDetectsSameSizeSameTimestampMetadataReplacement() throws {
        let directory = try makeModelDirectory()
        let validator = AFMMLXVisionAssetValidator()
        let architecture = try AFMMLXModelArchitecture.preflightConfiguration(
            in: directory,
        ...
        let processorURL = directory.appendingPathComponent("preprocessor_config.json")
        let originalAttributes = try FileManager.default.attributesOfItem(atPath: processorURL.path)
        let originalDate = try XCTUnwrap(originalAttributes[.modificationDate] as? Date)
        let originalData = try Data(contentsOf: processorURL)
        let originalText = try XCTUnwrap(String(data: originalData, encoding: .utf8))
        let replacementText = originalText.replacingOccurrences(of: "0.5", with: "0.0")
        XCTAssertEqual(originalText.utf8.count, replacementText.utf8.count)
        try Data(replacementText.utf8).write(to: processorURL)
        try FileManager.default.setAttributes(
            [.modificationDate: originalDate],
            ofItemAtPath: processorURL.path
        )
    ...
    func testLargeAcceptedMetadataInvalidatesCacheAtSameSizeAndTimestamp() throws {
        let directory = try makeModelDirectory()
        let processorURL = directory.appendingPathComponent("preprocessor_config.json")
        ...
        processor["padding"] = String(repeating: "x", count: 5 * 1_024 * 1_024)
        try Self.writeJSON(processor, to: processorURL)
        let originalDate = try XCTUnwrap(
            FileManager.default.attributesOfItem(atPath: processorURL.path)[.modificationDate]
                as? Date
        )
        let validator = AFMMLXVisionAssetValidator()
        let architecture = try AFMMLXModelArchitecture.preflightConfiguration(
        ...
        let first = validator.qualify(modelDirectory: directory, architecture: architecture)
        let originalData = try Data(contentsOf: processorURL)
        let originalText = try XCTUnwrap(String(data: originalData, encoding: .utf8))
        let replacementText = originalText.replacingOccurrences(of: "0.5", with: "0.0")
        XCTAssertEqual(originalText.utf8.count, replacementText.utf8.count)
        ...
        let second = validator.qualify(modelDirectory: directory, architecture: architecture)
