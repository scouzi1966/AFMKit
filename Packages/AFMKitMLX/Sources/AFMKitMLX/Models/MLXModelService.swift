    ...
    @unchecked Sendable
{
    ...
    private static var defaultGPUTraceDirectory: URL {
        ...
        return applicationSupport
            .appendingPathComponent("AFMKit", isDirectory: true)
            .appendingPathComponent("Traces", isDirectory: true)
    }

    private static var defaultGPUTraceOutputPath: String {
        defaultGPUTraceDirectory.appendingPathComponent("afm-metal.trace").path
    }

    private static var defaultGPUCaptureOutputPath: String {
        defaultGPUTraceDirectory.appendingPathComponent("afm-trace.gputrace").path
    }
    ...
    private func beginGPUTraceIfNeeded() -> Bool {
        ...
        let outputPath = ProcessInfo.processInfo.environment["AFM_GPU_TRACE_OUTPUT"]
            ?? Self.defaultGPUTraceOutputPath
    ...
    private func printGPUProfileFooter(promptTokens: Int, completionTokens: Int, promptTime: Double, generateTime: Double) {
        ...
        print("[\(ts())] [GPU-PROFILE]   GPU capture: afm mlx -m <small-model> --gpu-capture \(Self.defaultGPUCaptureOutputPath)")
    }
    ...
    public func ensureLoaded(
        ...
        try Self.validateForcedVisionSelection(
            forceVLM: forceVLM,
            modelID: modelID,
    ...
    public func generate(
        ...
        if mtpEligible {
            if let mtpResult = try await container.perform({ context -> (String, Int, Int)? in
                ...
                let lmInput = try await context.processor.prepare(input: scratch.userInput)
                guard AFMMLXMTPRuntimePolicy.shouldUseSpeculativeBinding(
                    requestedModelID: modelID,
                    mtpEnabled: self.mtpEnabled,
                    bindingModelID: mtpBinding?.modelID,
                    isMultimodal: self.isMultimodalInput(lmInput),
    ...
    static func validateForcedVisionSelection(
        forceVLM: Bool,
        modelID: String,
        ...
        throw MLXServiceError.visionAssetsUnavailable(
            model: modelID,
            missing: qualification.missingAssetNames
        )
    }
    ...
    public func validateMediaRequestCapabilities(
        ...
        guard let architecture = state.architecture,
            ...
            throw MLXServiceError.loadFailed(
                "\(modelID): loaded runtime capability state is unavailable"
            )
        }
